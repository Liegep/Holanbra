...
export function AdminAuthForm() {
  ...
  const handleLogin = async () => {
    ...
    try {
      ...
      setError(err.message || t('admin.auth.login_failed'));
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="space-y-6">
      <div className="bg-white/5 border border-white/10 p-12 rounded-3xl text-center space-y-8 flex flex-col items-center">
        ...
        <div className="space-y-2">
          <h3 className="text-white font-bold uppercase tracking-[0.3em] text-sm">{t('admin.auth.title')}</h3>
